...
namespace Utilities.PocoProperties
{
    public static class Poco
    {
            ...
            for (var col = 0; col <= table.Columns.Count - 1; col++)
            {
                try
                {
                    ...
                    else
                    {
                        ...
                        {
                            colNames.Remove(origColumnName);
                            item.SetProp(newColumnName, null, mappings);
                        }
                    }
        ...
        private static void SetProp<TItem>(this TItem item, string name, object value, Dictionary<string, string> mappings = null) where TItem : class
        {
            ...
            if ((name.Contains(".")))
            {
                ...
                try
                {
                    SetProp<object>(pitem, name.Replace(tstr[0] + ".", ""), value, mappings);
                }
                catch (Exception ex)
                {
                    var i = 0;
        ...
        public static void SetPropertyOn<TItem>(this TItem item, string propertyName, object value, Dictionary<string, string> mappings = null)
        {
            var headCol = propertyName;
            var column = value;


            if ((item.DoesPropertyExist(headCol)))
            {
                var tpe = item.GetPropertyType(headCol);
                try
                {
        ...
        public static void CopyInto<TItem>(this TItem item, TItem newItem)
        {
            ...
            foreach(var p in item.GetPropertyNames())
            {
        ...
        public static NameValueCollection PropertiesAsCollection<TItem>(this TItem item)
        {
            ...
            foreach (var p in item.GetPropertyNames(false))
            {
